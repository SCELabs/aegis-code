...
def test_readme_task_classifies_as_docs_task() -> None:
    assert classify_task_type("update README with examples for slugify and CLI usage") == "docs_task"


def test_mixed_fix_and_tests_task_classifies_as_implementation_with_tests() -> None:
    assert (
        classify_task_type("fix slugify so it strips punctuation and add tests for that")
...
def test_mixed_fix_and_tests_task_plan_targets_impl_and_tests(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", _provider)
    payload = build_run_payload(
        options=TaskOptions(task="fix slugify so it strips punctuation and add tests for that", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
    assert payload["patch_plan"]["task_type"] == "implementation_with_tests"
    assert "Prefer modifying tests only." not in str(payload["patch_plan"].get("strategy", ""))
    files = [item for item in payload["patch_plan"]["proposed_changes"] if isinstance(item, dict)]
    assert any(item.get("file") == "src/helpers.py" and item.get("change_type") == "modify" for item in files)
    assert any(
        item.get("file") == "tests/test_helpers.py" and item.get("change_type") in {"modify", "create"}
        for item in files
    )
...
def test_vague_task_skips_provider_with_task_too_vague(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    def _boom(**_: object):
        raise AssertionError("provider should not be called for vague tasks")

    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", _boom)
    payload = build_run_payload(
        options=TaskOptions(task="add a new feature with tests", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
    assert payload["patch_plan"]["task_type"] == "vague_task"
    assert payload["patch_diff"]["status"] == "skipped"
    assert payload["patch_diff"]["error"] == "task_too_vague"
    assert payload["patch_diff"]["attempted"] is False


def test_tagging_task_infers_explicit_targets(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", _provider)
    payload = build_run_payload(
        options=TaskOptions(task="add tagging support to todos with filtering and tests", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
    assert payload["patch_plan"]["task_type"] == "implementation_with_tests"
    ...
    assert "smaller first patch" in str(payload["patch_plan"].get("strategy", "")).lower()
    plan = captured.get("patch_plan")
    assert isinstance(plan, dict)
    assert sorted(plan.get("allowed_targets", [])) == ["src/main.py", "tests/test_cli.py"]
...
def test_readme_task_plans_readme_and_not_entrypoint(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    payload = build_run_payload(
        options=TaskOptions(task="update README with examples for slugify and CLI usage", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
    assert payload["patch_plan"]["task_type"] == "docs_task"
    files = [item for item in payload["patch_plan"]["proposed_changes"] if isinstance(item, dict)]
    assert any(item.get("file") == "README.md" and item.get("change_type") == "modify" for item in files)
    assert not any(item.get("reason") == "entrypoint_integration" for item in files)
    plan = captured.get("patch_plan")
    assert isinstance(plan, dict)
    assert plan.get("allowed_targets") == ["README.md"]
...
def test_docs_task_valid_readme_diff_is_generated(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    payload = build_run_payload(
        options=TaskOptions(task="update README with examples for cli usage", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
...
def test_docs_task_non_diff_is_wrapped_into_valid_readme_diff(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    assert payload["patch_diff"]["repair_reason"] == "docs_wrapped_diff"
    preview = str(payload["patch_diff"].get("preview", ""))
    assert "diff --git a/README.md b/README.md" in preview
    assert "+++ b/README.md" in preview
    diff_path = payload["patch_diff"].get("path")
    assert diff_path
    text = Path(str(diff_path)).read_text(encoding="utf-8")
    checked = check_patch_text(text, cwd=tmp_path)
    assert checked["valid"] is True
    assert checked["apply_blocked"] is False
    assert "README usage examples:" in text


def test_docs_task_provider_unavailable_non_diff_uses_wrapper_and_skips_regeneration(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    assert payload["patch_diff"]["status"] == "generated"
    assert payload["patch_diff"]["regeneration_attempted"] is False
    assert payload["patch_diff"]["repair_attempted"] is True
    assert payload["patch_diff"]["repair_applied"] is True
    assert payload["patch_diff"]["repair_status"] == "repaired"
    assert payload["patch_diff"]["repair_reason"] == "docs_wrapped_diff"
    path_value = payload["patch_diff"]["path"]
    assert path_value
    ...
    assert checked["valid"] is True
    assert checked["apply_blocked"] is False
    assert "Provider output did not look like a unified diff." not in text
    assert "# Project" in text


def test_docs_wrapper_uses_content_field_when_available(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    assert diff_path
    text = Path(str(diff_path)).read_text(encoding="utf-8")
    assert "Use `slugify` from CLI." in text


def test_docs_wrapper_empty_content_uses_template(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    text = Path(str(diff_path)).read_text(encoding="utf-8")
    assert "# Project" in text
    assert "- slugify example" in text


def test_readme_task_missing_readme_allows_create(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    assert payload["patch_plan"]["task_type"] == "docs_task"
    files = [item for item in payload["patch_plan"]["proposed_changes"] if isinstance(item, dict)]
    assert any(item.get("file") == "README.md" and item.get("change_type") == "create" for item in files)
    assert payload["patch_diff"]["status"] == "generated"


def test_docs_wrapped_diff_modify_and_create_headers(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_pass(), status="ok", exit_code=0),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "diff": "Updated README examples\n",
            "error": None,
        },
    )
    modify_payload = build_run_payload(
        options=TaskOptions(task="update README with examples for slugify and CLI usage", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
    modify_preview = str(modify_payload["patch_diff"].get("preview", ""))
    assert "--- a/README.md" in modify_preview
    assert "+++ b/README.md" in modify_preview

    # Create case
    (tmp_path / "README.md").unlink(missing_ok=True)
    create_payload = build_run_payload(
        options=TaskOptions(task="update README with examples for slugify and CLI usage", propose_patch=True),
        cwd=tmp_path,
        client=_Client(),
    )
    create_preview = str(create_payload["patch_diff"].get("preview", ""))
    assert "--- /dev/null" in create_preview
    assert "+++ b/README.md" in create_preview
...
def test_repaired_fallback_used_when_low_quality_regen_fails(monkeypatch, tmp_path: Path) -> None:
    ...
    assert payload["patch_diff"]["regeneration"]["final_status"] == "invalid"
...
def test_hard_invalid_regeneration_fails_still_invalid_and_once(monkeypatch, tmp_path: Path) -> None:
    ...
    assert calls["count"] == 1
    assert payload["patch_diff"]["status"] == "generated"
    assert payload["patch_diff"]["repair_applied"] is False
    assert payload["patch_diff"]["regeneration_attempted"] is False
    assert payload["patch_diff"]["regenerated"] is False
    assert payload["patch_diff"]["regeneration"]["attempt"] == 0


def test_regenerated_candidate_reports_repair_diagnostics(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_fail(), status="failed", exit_code=1),
    )
    ...
    payload = build_run_payload(options=TaskOptions(task="add feature", propose_patch=True), cwd=tmp_path, client=_Client())
    assert payload["patch_diff"]["regeneration_attempted"] is False
    assert payload["patch_diff"]["regenerated_repair_attempted"] is False
    assert payload["patch_diff"]["regenerated_repair_status"] == "not_attempted"
...
def test_provider_heartbeat_initial_generation_delay(monkeypatch, tmp_path: Path) -> None:
    ...
    assert progress
    assert any("provider" in m for m in progress)
...
def test_invalid_reason_fields_syntax_then_excessive_size(monkeypatch, tmp_path: Path) -> None:
    ...
    def _provider(**_: object) -> dict[str, object]:
        ...
        return {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "diff": f"diff --git a/tests/test_example.py b/tests/test_example.py\n--- a/tests/test_example.py\n+++ b/tests/test_example.py\n@@ -1 +1,302 @@\n-a\n+b\n{big_lines}",
            "error": None,
        }
    ...
    assert pd["regeneration_trigger_reason"] == "invalid_diff"
...
def test_provider_timeout_during_regeneration_sets_timeout_result(monkeypatch, tmp_path: Path) -> None:
    ...
    assert payload["patch_diff"]["status"] == "unavailable"
    assert payload["patch_diff"]["error"] == "provider_timeout"
    assert payload["patch_diff"]["regeneration"]["result"] == "unavailable"
    assert payload["patch_diff"]["regeneration"]["regenerated_invalid_reason"] == "provider_timeout"
    assert payload["patch_diff"]["invalid_diff_path"] is None
    assert not (tmp_path / ".aegis" / "runs" / "latest.diff").exists()
...
def test_patch_command_scope_missing_target_blocks_before_provider(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_fail(), status="failed", exit_code=1),
    )
    ...
    monkeypatch.setattr(
        "aegis_code.runtime.generate_structured_edits",
        lambda **_: (_ for _ in ()).throw(AssertionError("structured provider should not run")),
    )
    monkeypatch.setattr(
        "aegis_code.runtime.generate_patch_diff",
        lambda **_: (_ for _ in ()).throw(AssertionError("unified provider should not run")),
    )
    payload = build_run_payload(
        options=TaskOptions(
            ...
            scope_contract={
                "source": "cli_explicit",
                "allowed_targets": ["src/config.py"],
                "max_files": 1,
                "allow_new_files": False,
                "allowed_operations": ["replace"],
                "missing_targets": ["src/config.py"],
    ...
    assert payload["patch_diff"]["status"] == "blocked"
    assert payload["patch_diff"]["error"] == "requested_target_missing"
    assert payload["patch_diff"]["missing_targets"] == ["src/config.py"]


def test_patch_scope_extra_file_is_blocked(monkeypatch, tmp_path: Path) -> None:
    (tmp_path / "src").mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": '{"changes":[{"path":"src/main.py","mode":"replace","content":"x = 2\\n"},{"path":"tests/test_cli.py","mode":"create","content":"def test_x():\\n    assert True\\n"}]}',
            "error": None,
        },
    )
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", lambda **_: (_ for _ in ()).throw(AssertionError("fallback should not run")))
    ...
    assert payload["patch_diff"]["status"] == "blocked"
    assert payload["structured_patch"]["failure_reason"] in {"outside_allowed_targets", "invalid_path"}


def test_patch_scope_max_files_exceeded_is_blocked(monkeypatch, tmp_path: Path) -> None:
    (tmp_path / "src").mkdir(parents=True, exist_ok=True)
    (tmp_path / "tests").mkdir(parents=True, exist_ok=True)
    (tmp_path / "src" / "main.py").write_text("x = 1\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": '{"changes":[{"path":"src/main.py","mode":"replace","content":"x = 2\\n"},{"path":"tests/test_cli.py","mode":"replace","content":"def test_new():\\n    assert True\\n"}]}',
            "error": None,
        },
    )
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", lambda **_: (_ for _ in ()).throw(AssertionError("fallback should not run")))
    payload = build_run_payload(
        options=TaskOptions(
            task="add cli and tests",
            propose_patch=True,
            command="patch",
            scope_contract={
                "source": "cli_explicit",
                "allowed_targets": ["src/main.py", "tests/test_cli.py"],
                "max_files": 1,
                "allow_new_files": False,
                "allowed_operations": ["replace"],
                "missing_targets": [],
    ...
    assert payload["patch_diff"]["status"] == "blocked"
    assert payload["structured_patch"]["failure_reason"] == "too_many_files"


def test_patch_command_with_passing_tests_still_attempts_structured_provider(monkeypatch, tmp_path: Path) -> None:
    ...
    def _structured(**_: object) -> dict[str, object]:
        calls["count"] += 1
        return {
            "available": True,
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": '{"changes":[{"path":"src/main.py","mode":"replace","content":"VALUE = 2\\n"}]}',
            "error": None,
        }
...
def test_patch_command_neutral_wording_does_not_skip_no_patch_required(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "src" / "main.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("VALUE = 1\n", encoding="utf-8")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": '{"changes":[{"path":"src/main.py","mode":"replace","content":"VALUE = 3\\n"}]}',
            "error": None,
        },
    )
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", lambda **_: (_ for _ in ()).throw(AssertionError("fallback should not run")))
    payload = build_run_payload(
        options=TaskOptions(
            task="store todos in the user's home directory",
            propose_patch=True,
            command="patch",
            scope_contract={
                "source": "cli_explicit",
    ...
    assert payload["patch_diff"]["attempted"] is True
    assert payload["patch_diff"]["status"] == "generated"
    assert "No failures detected" not in str(payload["patch_plan"].get("strategy", ""))


def test_append_operation_generates_local_diff_without_unified_fallback(monkeypatch, tmp_path: Path) -> None:
    ...
    assert payload["patch_diff"]["status"] == "generated"
    assert payload["patch_diff"]["available"] is True
    assert payload["patch_operation"]["operation"] == "append"
    diff_path = Path(str(payload["patch_diff"]["path"]))
    diff_text = diff_path.read_text(encoding="utf-8")
    assert "+def test_todo_cli_append():" in diff_text
    assert "-def test_old():" not in diff_text
    assert "def test_old():" in target.read_text(encoding="utf-8")


def test_append_generated_diff_has_zero_deletions_and_apply_checks_and_safety(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    ...
    validation = payload["patch_diff"]["validation_result"]
    assert validation["summary"]["deletions"] == 0
    checked = check_patch_text(Path(str(payload["patch_diff"]["path"])).read_text(encoding="utf-8"), cwd=tmp_path)
    assert checked["apply_blocked"] is False
    assert "patch_safety" in payload
    assert payload["patch_safety"]["highest_severity"] in {"pass", "warn", "fail"}


def test_append_operation_blocks_invalid_provider_payload(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": '{"changes":[{"path":"tests/test_cli.py","mode":"replace","content":"X"}]}',
            "error": None,
        },
    )
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", lambda **_: (_ for _ in ()).throw(AssertionError("fallback should not run")))
...
def test_append_operation_rejects_unified_diff_text(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": "diff --git a/tests/test_cli.py b/tests/test_cli.py\n@@ -1 +1,2 @@\n+X\n",
            "error": None,
        },
    )
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", lambda **_: (_ for _ in ()).throw(AssertionError("fallback should not run")))
...
def test_append_mode_never_calls_structured_proposal_controller(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: (_ for _ in ()).throw(AssertionError("structured controller should not run for append")),
    )
    monkeypatch.setattr(
        "aegis_code.runtime.generate_structured_edits",
        lambda **_: {
...
def test_append_mode_malformed_output_is_append_output_invalid_not_structured(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    monkeypatch.setattr(
...
def test_append_empty_content_maps_to_no_append_needed_and_writes_no_diff(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert payload["patch_diff"]["status"] == "blocked"
    assert payload["patch_diff"]["error"] == "no_append_needed"
    assert payload["patch_diff"]["path"] is None
    assert not (tmp_path / ".aegis" / "runs" / "latest.diff").exists()


def test_append_python_syntax_invalid_blocks_and_writes_no_diff(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    monkeypatch.setattr(
    ...
    assert payload["patch_diff"]["status"] == "blocked"
    assert payload["patch_diff"]["error"] == "append_syntax_invalid"
    assert payload["patch_diff"]["path"] is None
    assert not (tmp_path / ".aegis" / "runs" / "latest.diff").exists()


def test_append_python_obvious_undefined_name_blocks_as_semantic_suspicious(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    monkeypatch.setattr(
...
def test_append_blocks_python_m_invented_module_conflict(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    src = tmp_path / "src"
    ...
    assert payload["patch_diff"]["plan_consistent"] is None
    assert payload["patch_diff"]["plan_missing_targets"] == []


def test_append_blocks_todo_file_env_override_conflict(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    src = tmp_path / "src"
...
def test_append_blocks_unsupported_id_field_assumption(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    src = tmp_path / "src"
...
def test_append_blocks_done_word_when_source_uses_checkboxes(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    src = tmp_path / "src"
    src.mkdir(parents=True, exist_ok=True)
    (src / "main.py").write_text(
        "def render(todo):\n    return \"[x]\" if todo.get(\"done\") else \"[ ]\"\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
...
def test_append_valid_source_aligned_main_path_and_checkboxes_passes(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "tests" / "test_cli.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("def test_old():\n    assert True\n", encoding="utf-8")
    src = tmp_path / "src"
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "provider": "openai",
            "model": "gpt-4.1-mini",
            "text": '{"content":"\\n\\ndef test_main_path_and_checkbox_output():\\n    cmd = [\\"python\\", \\"src/main.py\\", \\"list\\"]\\n    output = \\"[x] buy milk\\"\\n    assert \\"[x]\\" in output\\n    assert cmd\\n"}',
            "error": None,
        },
    )
    monkeypatch.setattr("aegis_code.runtime.generate_patch_diff", lambda **_: (_ for _ in ()).throw(AssertionError("fallback should not run")))
...
def test_non_append_path_does_not_use_append_sanity_checks(monkeypatch, tmp_path: Path) -> None:
    ...
    target.write_text("VALUE = 1\n", encoding="utf-8")
    monkeypatch.setattr(
        "aegis_code.runtime._append_python_sanity_error",
        lambda **_: (_ for _ in ()).throw(AssertionError("append sanity should not run")),
    )
    monkeypatch.setattr(
        "aegis_code.runtime.run_configured_tests",
        lambda _cmd, cwd=None: command_result_from_output(pytest_output_fail(), status="failed", exit_code=1),
    ...
    assert payload["patch_diff"]["error"] not in {"append_syntax_invalid", "append_semantic_suspicious"}
